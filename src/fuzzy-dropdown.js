...
// fuzzyDropdown edited by Chris Jensen to add support for disabled options
...
function fuzzyDropdownInit($, subSearch) {
	...
	$.fn.fuzzyDropdown = function(options) {
		...
		for (var i = 0; i < list.length; i++) {
			li = $('<li></li>').attr('data-val', list[i].value)
			                   .text(list[i].text);
			if (list[i].disabled) {
				li.addClass('s_unavailable');
			}
			$dropdownUL.append(li);
		}
		// add the no results element
		li = $('<li ></li>').attr('id', noResultsId)
		...
		$dropdownCont.on('click', 'li', function(e) {
			var $self = $(this);

			if($self.hasClass('s_unavailable')) {
				e.preventDefault();
				e.stopPropagation();
				return;
			}
